...
function parseSettingIndexes(stdout) {
  ...
  if (values.length < 2) throw new Error('Windows did not report the processor setting.');
  const [ac, dc] = values.slice(-2);
  if (![ac, dc].every((value) => Number.isInteger(value) && value >= 0 && value <= 100)) throw new Error('Windows reported a processor setting outside 0–100%.');
  return { ac, dc };
}
