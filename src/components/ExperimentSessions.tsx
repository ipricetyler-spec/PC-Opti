...
export function ExperimentSessions({ history, evidence, onNavigate, onCompare }: {
  ...
  return <section aria-label="Saved experiment sessions" className="mb-6 rounded-2xl border border-cyan-400/20 bg-slate-900/60 p-5">
    ...
    {importPreview && <div className="mt-3 space-y-2 rounded-lg border border-cyan-400/30 p-3"><h3 className="text-sm font-semibold">Review session import</h3><p className="text-xs text-slate-400">{importPreview.added} new sessions; {importPreview.skipped} identical sessions skipped. Imported decisions reset to undecided. Captures and audit records are not imported; links require local verification. No setting is applied or restored.</p><ul className="max-h-48 overflow-auto text-xs">{importPreview.sessions.filter(item => !sessions.some(saved => saved.id === item.id)).map(item => <li key={item.id} className="py-1">{item.archived ? '[Archived] ' : ''}{item.workload} · {item.changeDescription}</li>)}</ul><button className={button} disabled={storageBlocked || !importPreview.added} onClick={() => { try { const checked = previewSessionImport(sessions,importRaw); if (save(checked.sessions)) { setImportPreview(null); setImportRaw(''); if (!activeId) select(selectedSessionId(checked.sessions,null)); } } catch (reason) { setImportPreview(null); setImportRaw(''); setError(reason instanceof Error ? reason.message : 'The session list changed. Preview the import again.'); } }}>Merge reviewed sessions</button><button className={button} onClick={() => { setImportPreview(null); setImportRaw(''); }}>Cancel import</button></div>}
    ...
    {sessions.length > 0 && <label className="mt-4 block text-xs">Resume session<select aria-label="Resume session" className={field} value={activeId} onChange={(event) => select(event.target.value)}>{sessions.map((item) => <option key={item.id} value={item.id}>{item.archived ? '[Archived] ' : ''}{item.workload} — {item.changeDescription}</option>)}</select></label>}
    {active && <div className="mt-4 space-y-3">
      ...
      <div className="grid gap-3 md:grid-cols-3">{(['baseline','candidate'] as const).map((phase) => {
        ...
        return <div key={phase}><label className="text-xs">{phase === 'baseline' ? 'Baseline capture' : 'Candidate capture'}<select multiple size={4} className={field} value={selectedIds} onChange={(event) => { const ids = Array.from(event.target.selectedOptions,option => option.value); update({[`${phase}Ids`]:ids,[`${phase}Id`]:ids[0] || '',decision:'UNDECIDED'}); }}>{selectedIds.filter(id => !captures.some(item => item.captureId === id && item.status === 'COMPLETE' && item.stopReason === 'TIMED' && item.protocolComplete === true)).map(id => <option key={id} value={id}>Saved selection · read evidence to verify</option>)}{captures.filter(item => item.status === 'COMPLETE' && item.stopReason === 'TIMED' && item.protocolComplete === true).map(item => <option key={item.captureId} value={item.captureId} disabled={otherIds.includes(item.captureId)}>{item.target.name} · {new Date(item.startedAt).toLocaleString()}</option>)}</select></label><p className="mt-1 text-xs text-slate-400">{selectedIds.length} selected</p><button className={button} onClick={() => update({[`${phase}Ids`]:[],[`${phase}Id`]:'',decision:'UNDECIDED'})}>Clear {phase} captures</button></div>;
      })}
