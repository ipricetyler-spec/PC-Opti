...
export function DisplaySetupGuide({ onOpenMeasure, onTest, snapshot, discovery, evidence, onCompare, onImportPreview }: {
  ...
  return <section className="rounded-2xl border border-slate-700 bg-slate-900/70 p-5" aria-label="Display and GPU setup guide">
    ...
    <ol className="mt-4 list-decimal space-y-4 pl-5 text-sm leading-relaxed text-slate-300">
      <li><strong className="text-white">Confirm the intended display.</strong> Open Windows Settings → System → Display → Advanced display, select the gaming monitor and review the supported refresh rates. Choose a supported rate for your resolution and cable. Verify the game uses the same monitor and intended resolution; Windows dynamic refresh rate is a separate feature from game VRR.</li>
      ...
      <li><strong className="text-white">Choose synchronization deliberately.</strong> {goal === 'balanced'
        ? 'For a supported VRR display, confirm its monitor setting and G-SYNC / FreeSync configuration in the GPU vendor software. Compare within its supported range. Above that range, V-Sync and cap behavior matter; follow the vendor’s combination for this game.'
        : 'Compare V-Sync off against your baseline if tearing is acceptable. Higher delivered FPS can help response, but an uncapped workload may also increase GPU load, heat and queueing. Measure the actual tradeoff.'}</li>
      <li><strong className="text-white">Use one intentional cap and latency path.</strong> Compare a sustainable in-game frame cap before stacking multiple limiters. For supported NVIDIA games, start with in-game Reflex; its behavior takes precedence over driver Low Latency Mode. NVIDIA documents automatic below-refresh capping with G-SYNC, V-Sync and Reflex / Ultra Low Latency, so do not blindly add another cap. For Radeon, check current support: Anti-Lag 2 requires game integration. Do not force an unsupported feature through third-party hooks.</li>
